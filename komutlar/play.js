...
exports.run = async (client, message, args) => {
    ...
    .setColor("#579CF4")
    .setTitle(":x: Hata")
    .setDescription("-p [Bağlantı veya şarkı ismi]")
    ...
    if (!permissions.has('CONNECT')) {
      ...
      .setColor("#579CF4")
      .setDescription(`:x: Herhangi bir ses kanalına katılmak için yeterli iznim yok. Lütfen yetkilendirin!`)
      return message.channel.send(warningErr);
    }
    if (!permissions.has('SPEAK')) {
      const musicErr = new RichEmbed()
      .setColor("#579CF4")
      .setDescription(`:x: Kanalda konuşma iznim olmadığı veya mikrofonum kapalı olduğu için müziği açamıyorum/şarkı çalamıyorum.!`)
      ...
      try {
          ...
          .setColor("#579CF4")
          .setDescription(`:x: Aradığınız isimde şarkı bulunamadı!`) 
    ...
    async function handleVideo(video, message, voiceChannel, playlist = false) {
        ...
        .setColor("#579CF4")
        .setAuthor(`Çalınıyor..`, song.thumbnail)
